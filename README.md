# Django на Kubernetes з AWS## 📚 Додаткова документація

- [🚀 Швидкий старт](./docs/QUICKSTART.md) - мінімальний набір команд для розгортання
- [🏗️ Архітектура проекту](./docs/ARCHITECTURE.md) - детальний опис архітектури системи
- [🛠️ Розв'язання проблем](./docs/TROUBLESHOOTING.md) - гід по troubleshooting та діагностиці, RDS та ECR

Цей проект демонструє розгортання Django додатку в Kubernetes кластері на AWS з використанням:
- **AWS EKS** - керований Kubernetes кластер
- **AWS RDS** - PostgreSQL база даних
- **AWS ECR** - реєстр Docker образів
- **Terraform** - інфраструктура як код
- **Helm** - управління Kubernetes ресурсами

## 🏗️ Архітектура

```
┌─────────────────┐    ┌──────────────────┐    ┌─────────────────┐
│   LoadBalancer  │────│  EKS Cluster     │────│   RDS PostgreSQL│
│   (AWS ELB)     │    │  (2 Worker Nodes)│    │   (Керована БД) │
└─────────────────┘    └──────────────────┘    └─────────────────┘
                              │
                       ┌──────────────┐
                       │  Django Pods │
                       │  (2 Replicas)│
                       └──────────────┘
                              │
                       ┌──────────────┐
                       │  ECR Registry│
                       │ (Docker Image)│
                       └──────────────┘
```

## � Додаткова документація

- [🚀 Швидкий старт](./docs/QUICKSTART.md) - мінімальний набір команд для розгортання
- [🏗️ Архітектура проекту](./docs/ARCHITECTURE.md) - детальний опис архітектури системи

## �📋 Передумови

### Необхідне програмне забезпечення:
- **Terraform** >= 1.0
- **AWS CLI** >= 2.0
- **Docker** >= 20.0
- **kubectl** >= 1.25
- **Helm** >= 3.8
- **Git**

### AWS налаштування:
```bash
# Налаштування AWS креденшалів
aws configure
# або встановити змінні середовища:
export AWS_ACCESS_KEY_ID="ваш-ключ"
export AWS_SECRET_ACCESS_KEY="ваш-секретний-ключ"
export AWS_DEFAULT_REGION="us-east-1"
```

## 🚀 Швидкий старт

### 1. Клонування репозиторію
```bash
git clone <repository-url>
cd devops/akolvakh/l-7
```

### 2. Розгортання інфраструктури
```bash
# Ініціалізація Terraform
terraform init

# Планування розгортання
terraform plan

# Застосування конфігурації
terraform apply
```

### 3. Налаштування kubectl
```bash
# Автоматичне налаштування
./scripts/setup-kubectl.sh

# Або вручну
aws eks update-kubeconfig --region us-east-1 --name eks-cluster-lesson-7
```

### 4. Збірка та завантаження Docker образу
```bash
# Збірка для AMD64 архітектури
./scripts/build-and-push.sh

# Або з тегом версії
./scripts/build-and-push.sh v1.4
```

### 5. Розгортання додатку
```bash
# Повний процес розгортання
./scripts/deploy-app.sh

# Або покроково
helm upgrade --install django-app ./charts/django-app
```

### 6. Створення адміністратора
```bash
# Отримання назви поду
kubectl get pods

# Створення суперкористувача
kubectl exec -it <pod-name> -- python manage.py createsuperuser
```

## 📁 Структура проекту

```
l-7/
├── environments/dev/           # Terraform конфігурації
│   ├── main.tf                # Головна конфігурація
│   ├── variables.tf           # Змінні
│   ├── outputs.tf             # Виходи
│   └── backend.tf             # Бекенд налаштування
├── modules/                   # Terraform модулі
│   ├── vpc/                   # VPC мережа
│   ├── eks/                   # EKS кластер
│   ├── rds/                   # RDS база даних
│   └── ecr/                   # ECR реєстр
├── django/                    # Django додаток
│   ├── app/                   # Django проект
│   ├── Dockerfile            # Docker конфігурація
│   └── requirements.txt       # Python залежності
├── charts/django-app/         # Helm чарт
│   ├── templates/            # Kubernetes шаблони
│   ├── values.yaml           # Значення Helm
│   └── Chart.yaml            # Метадані чарту
├── scripts/                   # Автоматизовані скрипти
│   ├── setup-kubectl.sh      # Налаштування kubectl
│   ├── build-and-push.sh     # Збірка та публікація образу
│   └── deploy-app.sh          # Розгортання додатку
└── README.md                  # Цей файл
```

## ⚙️ Детальна конфігурація

### Terraform модулі

#### VPC модуль
Створює мережеву інфраструктуру:
- VPC з CIDR 10.0.0.0/16
- 3 публічні підмережі
- 3 приватні підмережі
- Internet Gateway
- Route Tables

#### EKS модуль
Налаштовує Kubernetes кластер:
- EKS Control Plane
- Worker Node Group (2 вузли t3.medium)
- IAM ролі та політики
- EBS CSI драйвер для storage
- Security Groups

#### RDS модуль
Створює PostgreSQL базу даних:
- RDS PostgreSQL 13
- Multi-AZ для високої доступності
- Automated backups
- Security Groups
- Subnet Groups

#### ECR модуль
Налаштовує Docker реєстр:
- Private ECR repository
- Lifecycle policies
- Access policies

### Helm чарт

#### Основні компоненти:
- **Deployment**: Django поди з автоскейлінгом
- **Service**: LoadBalancer для зовнішнього доступу
- **HPA**: Horizontal Pod Autoscaler
- **ConfigMap**: Конфігурація додатку
- **Secret**: Чутливі дані (паролі БД)

#### Конфігурація в values.yaml:
```yaml
replicaCount: 2                    # Кількість реплік
image:
  repository: <ecr-url>            # ECR репозиторій
  tag: "v1.3"                     # Тег образу
  pullPolicy: IfNotPresent        # Політика завантаження

service:
  type: LoadBalancer              # Тип сервісу
  port: 80                        # Зовнішній порт
  targetPort: 8000               # Внутрішній порт

resources:
  requests:
    cpu: 100m                     # Мінімальний CPU
    memory: 256Mi                 # Мінімальна пам'ять
  limits:
    cpu: 500m                     # Максимальний CPU
    memory: 512Mi                 # Максимальна пам'ять

autoscaling:
  enabled: true                   # Увімкнення автоскейлінгу
  minReplicas: 2                  # Мінімум реплік
  maxReplicas: 6                  # Максимум реплік
  targetCPUUtilizationPercentage: 70  # Цільове навантаження CPU
```

## 🔧 Налаштування Django

### Основні налаштування (settings.py):
- **WhiteNoise** - для статичних файлів
- **PostgreSQL** - через dj-database-url
- **Gunicorn** - WSGI сервер
- **Security headers** - для безпеки

### Змінні середовища:
- `DATABASE_URL` - підключення до PostgreSQL
- `DJANGO_DEBUG` - режим налагодження (false для продакшн)
- `DJANGO_SECRET_KEY` - секретний ключ
- `ALLOWED_HOSTS` - дозволені хости

## 📊 Моніторинг та логи

### Перевірка стану кластера:
```bash
# Статус подів
kubectl get pods

# Статус сервісів
kubectl get svc

# Статус HPA
kubectl get hpa

# Логи додатку
kubectl logs -f <pod-name>
```

### Перевірка метрик:
```bash
# Використання ресурсів
kubectl top pods

# Використання вузлів
kubectl top nodes
```

## 🔒 Безпека

### Налаштовані заходи безпеки:
- **Pod Security Context** - non-root користувач
- **Security Headers** - XSS, CSRF захист
- **Network Policies** - ізоляція мережевого трафіку
- **Secrets** - для чутливих даних
- **IAM ролі** - мінімальні необхідні дозволи

### Рекомендації:
- Регулярно оновлюйте образи
- Використовуйте Scanner для перевірки вразливостей
- Налаштуйте Network Policies
- Ротуйте секретні ключі

## 📈 Масштабування

### Горизонтальне масштабування:
- HPA автоматично збільшує/зменшує кількість подів
- Базується на навантаженні CPU (70%)
- Мін: 2 поди, Макс: 6 подів

### Вертикальне масштабування:
```bash
# Оновлення ресурсів
kubectl patch deployment django-app-django-app -p='{"spec":{"template":{"spec":{"containers":[{"name":"django-app","resources":{"limits":{"cpu":"1000m","memory":"1Gi"}}}]}}}}'
```

## 🛠️ Розв'язання проблем

### Часті проблеми:

#### Під не запускається:
```bash
kubectl describe pod <pod-name>
kubectl logs <pod-name>
```

#### Проблеми з підключенням до БД:
```bash
# Перевірка секретів
kubectl get secrets

# Перевірка підключення
kubectl exec -it <pod-name> -- python manage.py check --database default
```

#### Проблеми зі статичними файлами:
```bash
# Перевірка збору статики
kubectl exec -it <pod-name> -- python manage.py collectstatic --dry-run
```

### Корисні команди:
```bash
# Перезапуск deployment
kubectl rollout restart deployment django-app-django-app

# Відкат до попередньої версії
kubectl rollout undo deployment django-app-django-app

# Масштабування вручну
kubectl scale deployment django-app-django-app --replicas=4
```

## 🔄 CI/CD Pipeline

### Рекомендований workflow:
1. **Розробка** - локальні зміни
2. **Тестування** - автоматичні тести
3. **Збірка** - Docker образ
4. **Деплой в Staging** - тестове середовище
5. **Деплой в Production** - продакшн середовище

### GitHub Actions приклад:
```yaml
name: Deploy to EKS
on:
  push:
    branches: [main]
jobs:
  deploy:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v3
      - name: Configure AWS credentials
        uses: aws-actions/configure-aws-credentials@v2
        with:
          aws-access-key-id: ${{ secrets.AWS_ACCESS_KEY_ID }}
          aws-secret-access-key: ${{ secrets.AWS_SECRET_ACCESS_KEY }}
          aws-region: us-east-1
      - name: Build and push
        run: ./scripts/build-and-push.sh ${{ github.sha }}
      - name: Deploy to EKS
        run: ./scripts/deploy-app.sh
```